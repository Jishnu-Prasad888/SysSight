        ...
        try:
            ...
            for log_entry in log_entries:
                try:
                    ...
                    # Save user sessions with proper validation
                    ...
                    if isinstance(users_logged_in, dict):
                        for user_data in users_logged_in.get('users', []):
                            try:
                                ...
                                # Validate and clean host field
                                host = user_data.get('host', '0.0.0.0')
                                # Handle IPv6 addresses and invalid formats
                                if host in [':1', '::1']:
                                    host = '127.0.0.1'  # Convert IPv6 localhost to IPv4
                                elif not host or host == ':0.0.0.0' or host.startswith(':'):
                                    host = '0.0.0.0'  # Default for invalid/empty hosts
                                
                                # Validate host is a proper IP address
                                try:
                                    # Basic IP validation
                                    if host.count('.') == 3 and all(part.isdigit() and 0 <= int(part) <= 255 for part in host.split('.')):
                                        valid_host = host
                                    else:
                                        valid_host = '0.0.0.0'
                                except:
